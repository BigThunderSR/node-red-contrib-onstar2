...
describe('locate-vehicle Node', function () {
  let sandbox;
  ...
  it('Should be loaded', function (done) {
    var flow = [
      { id:"n1",type:"locate-vehicle",name:"Locate Vehicle",onstar2:"Onstar Config",wires:[["n2"],["n3"]] }
    ];
    helper.load(onStar, flow, function () {
      var n1 = helper.getNode("n1");
      try {
        n1.should.have.property('name', 'Locate Vehicle');
        n1.should.have.property('type', 'locate-vehicle');
        done();
      } catch(err) {
        done(err);
      }
  ...
  it('Should output error with mocked authentication failure', function (done) {
    var mockClient = {
      location: sinon.stub().rejects(new Error('Unauthorized'))
    };
    sandbox.stub(OnStarJS, 'create').returns(mockClient);

    var flow = [
      { id:"n1",type:"locate-vehicle",name:"Test Node",onstar2:"17dfaade45168b46",wires:[["n2"],["n3"]] },
