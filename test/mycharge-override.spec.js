...
describe('mycharge-override', function () {
  ...
  it('Should be loaded', function (done) {
    var flow = [
      { id:"n1",type:"mycharge-override",name:"Charge Override",onstar2:"Onstar Config",wires:[["n2"],["n3"]] }
    ];
    helper.load(onStar, flow, function () {
      var n1 = helper.getNode("n1");
      try {
        n1.should.have.property('name', 'Charge Override');
        n1.should.have.property('type', 'mycharge-override');
        done();
      } catch(err) {
        done(err);
      }
  ...
  it('Should output 401 error due to not having valid credentials', function (done) {
    var flow = [
      { id:"n1",type:"mycharge-override",name:"Charge Override",onstar2:"17dfaade45168b46",wires:[["n2"],["n3"]] },
