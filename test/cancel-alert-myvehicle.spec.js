...
describe('cancel-alert-myvehicle Node', function () {
  let sandbox;
  ...
  it('Should be loaded', function (done) {
    var flow = [
      { id:"n1",type:"cancel-alert-myvehicle",name:"Cancel Alert Vehicle",onstar2:"Onstar Config",wires:[["n2"],["n3"]] }
    ];
    helper.load(onStar, flow, function () {
      var n1 = helper.getNode("n1");
      try {
        n1.should.have.property('name', 'Cancel Alert Vehicle');
        n1.should.have.property('type', 'cancel-alert-myvehicle');
        done();
      } catch(err) {
        done(err);
      }
  ...
  it('Should output error with mocked authentication failure', function (done) {
    var mockClient = {
      cancelAlert: sinon.stub().rejects(new Error('Unauthorized'))
    };
    sandbox.stub(OnStarJS, 'create').returns(mockClient);

    var flow = [
      { id:"n1",type:"cancel-alert-myvehicle",name:"Test Node",onstar2:"17dfaade45168b46",wires:[["n2"],["n3"]] },
