...
describe('get-account-vehicles Node', function () {
  let sandbox;
  ...
  it('Should be loaded', function (done) {
    var flow = [
      { id:"n1",type:"get-account-vehicles",name:"Get Account Vehicles",onstar2:"Onstar Config",wires:[["n2"],["n3"]] }
    ];
    helper.load(onStar, flow, function () {
      var n1 = helper.getNode("n1");
      try {
        n1.should.have.property('name', 'Get Account Vehicles');
  ...
  it('Should output error with mocked authentication failure', function (done) {
    // Mock OnStar.create to return a client that rejects with auth error
    var mockClient = {
      getAccountVehicles: sinon.stub().rejects(new Error('Unauthorized'))
    };
    sandbox.stub(OnStarJS, 'create').returns(mockClient);

    var flow = [
      { id:"n1",type:"get-account-vehicles",name:"Get Account Vehicles",onstar2:"17dfaade45168b46",wires:[["n2"],["n3"]] },
