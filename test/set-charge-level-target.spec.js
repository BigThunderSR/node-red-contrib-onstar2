...
describe('set-charge-level-target', function () {
  ...
  it('Should be loaded', function (done) {
    var flow = [
      { id:"n1",type:"set-charge-level-target",name:"Set Charge Level Target",onstar2:"Onstar Config",targetlevel:"80",wires:[["n2"],["n3"]] }
    ];
    helper.load(onStar, flow, function () {
      var n1 = helper.getNode("n1");
      try {
        n1.should.have.property('name', 'Set Charge Level Target');
        n1.should.have.property('type', 'set-charge-level-target');
        done();
      } catch(err) {
        done(err);
      }
  ...
  it('Should output error with mocked authentication failure', function (done) {
    var mockClient = {
      setChargeLevelTarget: sinon.stub().rejects(new Error('Unauthorized'))
    };
    sandbox.stub(OnStarJS, 'create').returns(mockClient);

    var flow = [
      { id:"n1",type:"set-charge-level-target",name:"Test Node",onstar2:"17dfaade45168b46",wires:[["n2"],["n3"]] },
