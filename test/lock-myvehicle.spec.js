...
describe('lock-myvehicle Node', function () {
  let sandbox;
  ...
  it('Should be loaded', function (done) {
    var flow = [
      { id:"n1",type:"lock-myvehicle",name:"Lock Vehicle",onstar2:"Onstar Config",delay:"",wires:[["n2"],["n3"]] }
    ];
    helper.load(onStar, flow, function () {
      var n1 = helper.getNode("n1");
      try {
        n1.should.have.property('name', 'Lock Vehicle');
        n1.should.have.property('type', 'lock-myvehicle');
        done();
      } catch(err) {
        done(err);
      }
  ...
  it('Should output error with mocked authentication failure', function (done) {
    var mockClient = {
      lockDoor: sinon.stub().rejects(new Error('Unauthorized'))
    };
    sandbox.stub(OnStarJS, 'create').returns(mockClient);

    var flow = [
      { id:"n1",type:"lock-myvehicle",name:"Test Node",onstar2:"17dfaade45168b46",wires:[["n2"],["n3"]] },
